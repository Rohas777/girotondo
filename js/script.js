...
$(document).ready(function () {
    ...
    $(".events__slider.owl-carousel").owlCarousel({
        items:  3, // Количество элементов в слайде
        loop: true, // Включение бесконечного цикла
    ...
    $(".events__slider.owl-carousel").on("changed.owl.carousel", function(event) {
        $('.events__slider .owl-item').each(function(index){
            $(this).removeClass('sized');
        })
        $('.events__slider .owl-item').eq(event.item.index + 1).addClass('sized');
    });
